...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=darth-twitter.example.com
...
workon darth-twitter  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development darth-twitter npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop darth-twitter
    docker rm darth-twitter
    docker run --name darth-twitter -d --env-file /home/ubuntu/darth-twitter.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/darth-twitter:/dkdata \
        darth-twitter start_web.sh
}
...
echo_green "Welcome to darth-twitter's dev env"
